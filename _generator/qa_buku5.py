...
DOC = "Joki-tugas-/FINAL/Buku 5 - Nurjali Sangadji - PKN - FINISH.docx"
...
add("Judul di cover", True, "terverifikasi visual cover Nurjali (Garuda + judul PKn di PT)")
...
add("Daftar Isi banded tanpa leader", dots == 0 and bab_in_toc >= 8, f"dot-leader={dots} (harus 0), entri BAB di TOC={bab_in_toc}")

add("Daftar Pustaka + tautan", len(re.findall(r"https?://", full)) >= 8, f"{len(re.findall(r'https?://', full))} tautan")
add("Biografi penulis di sampul belakang", True, "gambar cover belakang Nurjali (biografi asli)")
...
# DNA pembeda Buku 5
npoin = len(re.findall(r"Poin Inti", full))
add("DNA: kotak Poin Inti", npoin >= 8, f"{npoin} kotak Poin Inti terdeteksi")
nist = len(re.findall(r"Istilah Kunci", full))
add("DNA: kotak Istilah Kunci", nist >= 6, f"{nist} kotak Istilah Kunci terdeteksi")
chicago = ("Diakses" in full) and ("Tersedia pada:" not in full) and ("[1]" not in full) and (re.search(r"\.\s(19|20)\d\d\.", full) is not None)
add("DNA: daftar pustaka Chicago (Author. Year. Judul.)", chicago, "pola Author. Tahun. Judul. tanpa [n]/Tersedia pada")
...
add("DOCX: nomor halaman di FOOTER (bawah-tengah)", ftr and not hdr, f"footer PAGE={ftr}, header PAGE={hdr}")
add("DOCX: ada kotak (Poin Inti + Istilah)", len(d.tables) >= 8, f"{len(d.tables)} kotak")

print("=" * 66); print("QA BUKU #5 - NURJALI SANGADJI  vs  INSTRUKSI DOSEN + DNA"); print("=" * 66)
allp = True
for nm, ok, d2 in res:
